...
//     if (error?.response?.data?.errorCode === "SUBSCRIPTION_EXPIRED") {
//       return false;
//     }
...
//       });
//     } catch (requestError) {
//       if (requestError.response?.data?.errorCode === "SUBSCRIPTION_EXPIRED") {
...
const resolveLoginType = (value = "", endpoint = "") => {
  ...
  if (endpoint === API_ENDPOINTS.auth.superAdminLogin) return "super-admin";
  if (endpoint === API_ENDPOINTS.auth.adminLogin) return "admin";
  if (endpoint === API_ENDPOINTS.auth.userLogin) return "user";

  if (["superadmin", "superadministrator"].includes(normalized)) return "super-admin";
  if (["admin", "administrator"].includes(normalized)) return "admin";
  if (["onboarding", "candidate"].includes(normalized)) return "onboarding";

  return normalized ? "user" : "";
};

// -------------------------------------------------------------
// Helper: Thorough check for subscription expiration
// -------------------------------------------------------------
const isSubscriptionExpiredError = (error) => {
  if (!error?.response) return false;

  const status = error.response.status;
  const data = error.response.data;

  // HTTP 402: Payment Required (standard subscription error status)
  if (status === 402) return true;

  const errorCode = String(data?.errorCode || data?.code || data?.error_code || "").toUpperCase();
  if (errorCode === "SUBSCRIPTION_EXPIRED" || errorCode === "SUBSCRIPTION_INACTIVE") {
    return true;
  }

  if (data?.isSubscriptionExpired === true || data?.subscriptionExpired === true) {
    return true;
  }

  const message = String(
    data?.message || data?.error || (typeof data === "string" ? data : "") || error.message || ""
  ).toLowerCase();
...
export default function LoginLeft() {
  ...
  const [rememberMe, setRememberMe] = useState(false);
  const [error, setError] = useState("");
  const [subscriptionError, setSubscriptionError] = useState(null); // { title: string, message: string }
  const [loading, setLoading] = useState(false);
  const [showPassword, setShowPassword] = useState(false);
  ...
  const shouldFallbackToUserLogin = (error) => {
    // 1. NEVER fallback to user login if subscription has expired
    ...
    // 2. Also avoid fallback if message indicates subscription failure
  ...
  const submitLoginRequest = async () => {
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        // If subscription is expired on adminLogin, stop immediately!
        if (
          endpoint !== API_ENDPOINTS.auth.userLogin &&
          shouldFallbackToUserLogin(error)
        ) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const resolvedEmployeeId = employeeId || userId || attendanceId;
      ...
      storage.setItem("token", token);
      localStorage.setItem("loginTime", String(Date.now()));
      storage.setItem("role", isOnboardingLogin ? "onboarding" : authenticatedRole);
      storage.setItem("roleName", isOnboardingLogin ? "Onboarding" : roleName);
      storage.setItem("roleId", roleId || "");
      storage.setItem("email", form.email);
      storage.setItem("loginType", isOnboardingLogin ? "onboarding" : loginType || "");
      storage.setItem("userType", isOnboardingLogin ? "onboarding" : loginType || "");
      ...
      if (organizationId) storage.setItem("organizationId", organizationId);
      if (branchId) storage.setItem("branchId", branchId);
      if (companyId) storage.setItem("companyId", companyId);
      if (tenantId) storage.setItem("tenantId", tenantId);
      ...
      // Normal flow if subscription is active:
      if (!isOnboardingLogin) {
        if (shouldUseAdminPermissions) {
          await refreshAdminPermissions({ force: true });
        } else {
      ...
      // ---------------------------------------------------------
      if (isSubscriptionExpiredError(requestError)) {
        const backendMessage =
          requestError.response?.data?.message ||
          requestError.response?.data?.error;

        setSubscriptionError({
          title: "Subscription Pack Expired",
          message:
            backendMessage ||
            "Your company's subscription pack has ended. Please renew your plan or contact your administrator to restore workspace access."
        });
        return;
      }
