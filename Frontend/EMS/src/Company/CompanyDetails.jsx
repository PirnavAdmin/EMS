...
function CompanyDetails() {
  ...
  const validateCompany = () => {
    ...
    if (!isMeaningfulName(companyName)) {
      nextErrors.name =
    ...
    if (isFutureDate(company.established)) {
      nextErrors.established =
    ...
    if (!/^\d{10}$/.test(company.phone)) {
      nextErrors.phone =
      "Phone Number must contain exactly 10 digits";
    } else
    if (/^(\d)\1{9}$/.test(company.phone)) {
      nextErrors.phone =
    ...
    if (!/^GSTIN\d{10}$/.test(company.gst)) {
      nextErrors.gst =
  ...
  const validateBranch = () => {
    ...
    if (!isMeaningfulName(branchName)) {
      nextErrors.name =
    ...
    if (/^(\d)\1{9}$/.test(branchForm.phone)) {
      nextErrors.phone =
      "Phone Number cannot contain same digit continuously";
    }
