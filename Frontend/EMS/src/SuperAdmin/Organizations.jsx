/* --- old: Admin Management table/form, replaced by Organizations ---
...
import React, { useEffect, useState } from "react";
/* --- old: organization page icon imports ---
            ...
            */}
            ...
            <div className="org-section">
              ...
              */}
              ...
              {/* --- old: shared table-action styling for the Add Admin control ---
              {!showAdminFields && <div className="org-admins-empty"><FaUser /><span>No administrators added yet.</span><button className="emp-action-btn emp-action-btn--edit" type="button" onClick={() => setShowAdminFields(true)}>+ Add Admin</button></div>}
              */}
              {/* --- old: one-admin empty state and Add Admin toggle ---
              {!showAdminFields && <div className="org-admins-empty"><FaUser /><span>No administrators added yet.</span><button className="org-add-admin-btn" type="button" onClick={() => setShowAdminFields(true)}><FaPlus /> Add Admin</button></div>}
              */}
              {/* --- old: administrator fields without entry hints ---
              {showAdminFields && <div className="admin-fields-grid">
                <label>Admin Email<div className="org-input-wrap"><FaEnvelope className="org-field-icon" /><input name="adminEmail" type="email" value={form.adminEmail} onChange={handleChange} required /></div></label>
                <label>Admin Password<div className="org-input-wrap"><FaShieldAlt className="org-field-icon" /><input name="adminPassword" type="password" value={form.adminPassword} onChange={handleChange} required /></div></label>
              </div>}
              */}
              {/* --- old: fixed single Admin Email/Admin Password fields ---
              {showAdminFields && <div className="admin-fields-grid">
                <label>Admin Email<div className="org-input-wrap"><FaEnvelope className="org-field-icon" /><input name="adminEmail" type="email" placeholder="admin@company.com" value={form.adminEmail} onChange={handleChange} required /></div></label>
                <label>Admin Password<div className="org-input-wrap"><FaShieldAlt className="org-field-icon" /><input name="adminPassword" type="password" placeholder="Enter admin password" value={form.adminPassword} onChange={handleChange} required /></div></label>
              </div>}
              */}
              {admins.length === 0 && <div className="org-admins-empty"><FaUser /><span>No administrators added yet.</span></div>}
              {/* --- old: two-field administrator row ---
              {admins.map((admin, index) => (
                <div className="admin-fields-grid" key={`admin-${index}`}>
                  ...
                  <button className="emp-action-btn emp-action-btn--delete" type="button" onClick={() => setAdmins((current) => current.filter((_, entryIndex) => entryIndex !== index))}>Remove</button>
                </div>
              ))}
              <button className="org-add-admin-btn" type="button" onClick={() => setAdmins((current) => [...current, { email: "", password: "" }])}><FaPlus /> Add Admin</button>
              */}
              {admins.map((admin, index) => (
                <div className="admin-block" key={`admin-${index}`}>
                  ...
                  {/* --- old: left-aligned Remove button directly below the fields ---
                  <button className="emp-action-btn emp-action-btn--delete" type="button" onClick={() => setAdmins((current) => current.filter((_, entryIndex) => entryIndex !== index))}>Remove</button>
                  */}
                  <div className="admin-block-footer">
                    <button className="remove-btn" type="button" onClick={() => setAdmins((current) => current.filter((_, entryIndex) => entryIndex !== index))}>Remove</button>
                  </div>
                </div>
              ))}
              {/* --- old: Add Admin button appending an email/password-only entry ---
              <button className="org-add-admin-btn" type="button" onClick={() => setAdmins((current) => [...current, { email: "", password: "" }])}><FaPlus /> Add Admin</button>
              */}
              {/* --- old: Add Admin button positioned below all administrator blocks ---
              <button className="org-add-admin-btn" type="button" onClick={() => setAdmins((current) => [...current, { fullName: "", phoneNumber: "", email: "", password: "" }])}><FaPlus /> Add Admin</button>
              */}
              {/* --- old: create-only administrators section closing conditional ---
            </div>}
            */}
            </div>
            <div className="emp-modal-btns">
